...
shortDescription: |
  MongoDB is technically a database.

# createdAt: '2025-09-28'
# updatedAt: '2025-09-28'
---

<Note variant="info" title="">

The opinions expressed in this post do not reflect those of my employer :)

</Note>

I've used MongoDB ~too much~ a good amount over the last few years at work, and
in a few [side](https://msg.soorria.com) [projects](https://jjs.soorria.com)
before. The more I use it, the more disappointed I am in how the database works,
and its ergonomics.

Below is a series of complaints I have about the MongoDB database and the
...
`path.to.data`, you instead query / update the following field:
...
However, when you insert a document with the field `path.to.data` (e.g. using
`insertOne` or `insertMany`), the resulting document is:
...
Technically, this point is a lie and you _can_ filter documents with dotted
fields using something disgusting like this:
...
- unfathomable amounts of layout shift when loading a collection
  ...
  - while writing this post, they have a new preview UI that uses the same UI as
    the desktop app

## Paid support is required
...
## JavaScript `undefined`s become `null`s 🙃
...
If for whatever reason, you're emotionally attached to MongoDB and can't stand
deleting it, you can set `ignoreUndefined: true` when you create an instance of
the torture device:
