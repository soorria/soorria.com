...
const isRehypeCodeWrapper = (n: Node): n is Element =>
...
export const rehypeRearrangePrettyCodeOutput: Plugin = () => {
  return (tree: UnistNode) =>
    visit(
      tree,
      isRehypeCodeWrapper,
      (node: Element, index: number, parent: Node & { children: Element[]; name?: string }) => {
        const isInTsJsSwitcher =
          parent.type === 'mdxJsxFlowElement' && parent.name === 'TsJsSwitcher'
        ...
        const extraProperties: Partial<
          Record<'data-wrapper' | 'data-title' | 'data-caption', string>
        > = {}
        ...
        if (maybeCaptionEl) {
          extraProperties['data-caption'] = (
            maybeCaptionEl.children![0] as { value: string }
          )?.value
        }
