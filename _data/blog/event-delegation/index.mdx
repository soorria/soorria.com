...
we need to add an event listener to the parent:
...
_could_ just remove the icons, but they look nice and that's important to us.
Instead, we'll use the
[`closest`](https://developer.mozilla.org/en-US/docs/Web/API/Element/closest)
method available on all HTML elements. It accepts a CSS selector (like
`querySelector`) and walks up the DOM tree until it finds an element that
matches the selector, or runs our of elements. This allows us to the get the
...
Notice above that we return early `event.target.closest` if returns a nullish
...
since their values are easier to access in JavaScript.

You can use any data attribute, but since the buttons are for bending elements
from
...
Now, we can access the value of the `data-element` attribute using there dataset
...
Here's a demo with the icons, and _a_ complete, useful and non-trivial
`showAlertForElement`:
