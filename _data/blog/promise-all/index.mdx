...
right after the for-loop means the promise gets resolved with an empty array.
...
```ts {1-2,4-5,7-8,10} jsKeepLines
// ...
const results: PromiseAllResult<Promises> = Array(promises.length)
...
import { WaitingInOrderExample } from './components'

<Demo component={WaitingInOrderExample} />

## Handling errors

We're finally almost done - we just need to handle errors. `Promise.all` throws
an error if _any of the provided promises reject_. We can accomplish this by
calling `reject` with the error when any of the provided promises throw:

```ts {15-17} jsKeepLines
// ...
const results: PromiseAllResult<Promises> = Array(promises.length)
let numResolvedPromises = 0
