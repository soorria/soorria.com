...
if (typeof window !== 'undefined') {
  ...
  window.addEventListener('storage', event => {
    if (
      // the `storage` event also fires for `sessonStorage`, and we don't care
      // about that for this hook
      event.storageArea === localStorage &&
      // Intentionally using `!=` instead of `!==` since it checks `null`
      // and `undefined`
      event.key != null &&
...
- This hook relies on the [`mitt`](https://github.com/developit/mitt) event
  emitter package - it's only tiny at only 200 bytes(!!) that I would have had
  to include anyway (for syncing between hook instances in the same tab), and
  has [tests](https://github.com/developit/mitt/blob/main/test/index_test.ts)
...
- The type of the `state` that you use this hook with cannot be nullable (i.e.
  cannot be `null` or `undefined`). This is enforced in the type signature of
  the hook so if you're using TypeScript, issues should be caught at
  compile-time. If you do want the value to be nullable, I recommend wrapping
  your state in an object (e.g.
