...
(i.e. the callback argument passed to `new Promise`). This is mostly just useful
for making your code 1 level less nested, or if you're allergic to `async` and
`await` syntax.
...
export const fetchPosts = (): Promise<Post[]> => {
  const { promise, resolve } = createManualPromise<Post[]>()
