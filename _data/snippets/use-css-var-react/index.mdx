...
Set, get and remove a [CSS custom property](https://developer.mozilla.org/en-US/docs/Web/CSS/Using_CSS_custom_properties)
from the comfort of a React component. This _could_ result in slightly more performant
code when you're setting the `style` property directly since React will re-run your
component fewer times.
...
- This hook doesn't use `useState` since it would nullify the performance aspect,
  and I didn't feel the need to be notified when the variable changed on the root.
  You could use [`MutationObserver`](https://developer.mozilla.org/en-US/docs/Web/API/MutationObserver)
...
<Demo component={Example} init="lazy" />
