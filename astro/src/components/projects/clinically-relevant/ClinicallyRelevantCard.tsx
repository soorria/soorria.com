...
const cardLinkClassName = (theme: Theme) =>
  cx(
    'px-3 py-1 ml-2 mt-2 inline-flex space-x-1 text-sm items-center border-2 transition border-current rounded-xs font-semibold focus-ring',
    theme === 'light'
      ? 'hocus:bg-gray-800 hocus:text-gray-100 hocus:border-gray-800'
      : 'hocus:bg-gray-100 hocus:text-gray-800 hocus:border-gray-100'
  )

const ClinicallyRelevantCard: React.FC<ProjectCardProps> = ({ project }) => {
  const trackFirstEvent = useTrackFirstEvent()
  const [theme, toggle] = useReducer(
    (t: Theme) => {
      trackFirstEvent('Easter Egg', { props: { which: 'clinically-relevant-dark-mode' } })
      ...
      <div className={cx(COMMON_CLASSNAMES.specialCardRoot, themeClasses[theme])}>
        <div className="flex h-full flex-col space-y-4 p-8">
          ...
          <div className="-m-2 mb-0">
            {project.hasContent && (
              <a href={`/projects/${project.slug}`} className={cardLinkClassName(theme)}>
